...
public class BaseController {
	 ...
	 *            参数map形式如 {'name':'name不能为空'}
	 * @return
	 */
	public boolean validateParamBlank(HttpServletRequest request,
			HttpServletResponse response, Map<String, String> validateResult) {
		if (validateResult == null)
			return true;

		Set<String> keys = validateResult.keySet();
		for (String key : keys) {
			String param = request(request, key);
			if (StringUtils.isBlank(param)) {
				JSONUtils.PARAM_ERROR(response, validateResult.get(key));
				return false;
			}
		}
		return true;
